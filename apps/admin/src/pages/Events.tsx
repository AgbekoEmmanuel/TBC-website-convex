    ...
    <Card className={cn(
      ...
      <div className="flex-1 min-w-0 mt-2 md:mt-0 px-2 md:px-0">
        <div className="flex items-center gap-2 mb-1 flex-wrap">
          ...
          {ev.isPublished ? (
            <span className="text-[9px] bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 px-2 py-0.5 rounded-full font-bold uppercase tracking-wider">Live</span>
          ) : (
            <span className="text-[9px] bg-slate-200 text-slate-600 dark:bg-white/10 dark:text-slate-400 px-2 py-0.5 rounded-full font-bold uppercase tracking-wider">Draft</span>
          )}
          {ev.date ? (
            ev.date >= new Date().toISOString().split("T")[0] ? (
              <span className="text-[9px] bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 px-2 py-0.5 rounded-full font-bold uppercase tracking-wider">Upcoming</span>
            ) : (
              <span className="text-[9px] bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 px-2 py-0.5 rounded-full font-bold uppercase tracking-wider">Past</span>
            )
          ) : null}
          {ev.isFeatured && (
      ...
      <div className="hidden sm:flex items-center gap-0">
         ...
         <div className="flex flex-col items-end justify-center w-[140px] lg:w-[160px] flex-shrink-0 pr-6 h-10">
            <span className="text-[14px] font-medium text-[#112a46] dark:text-white leading-tight mb-[2px]">{ev.category ? (ev.category.toLowerCase() === "special" ? "Special Programs" : ev.category.charAt(0).toUpperCase() + ev.category.slice(1)) : "Uncategorized"}</span>
...
export function Events() {
  ...
  const filteredEvents = events?.filter(ev => {
    // Timeframe filter
    if (timeFilter === "upcoming" && ev.date < today) return false;
    if (timeFilter === "past" && ev.date >= today) return false;
    ...
    return cat === tab || 
           (tab === "special programs" && (cat === "special" || cat === "special programs")) ||
           (tab === "special" && (cat === "special" || cat === "special programs"));
  });
  ...
  return (
    <div className="space-y-8 max-w-[1400px] mx-auto px-1 sm:px-2 md:px-0 text-[#112a46] dark:text-white mb-20 animate-in fade-in duration-300 pt-2">
      ...
      <div className="flex flex-col">
         <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-200 dark:border-white/5 pb-5 mb-6 gap-4">
            <div className="flex items-center gap-3 flex-wrap">
              <h2 className="text-[26px] font-serif text-[#112a46] dark:text-white font-bold leading-none tracking-tight">Events List</h2>
              <div className="flex items-center bg-slate-100 dark:bg-white/5 p-1 rounded-xl">
                 {(["all", "upcoming", "past"] as const).map((mode) => (
                   <button
                     ...
                     className={cn(
                       "text-[12px] font-semibold px-3 py-1 rounded-lg transition-all capitalize",
                       timeFilter === mode 
                         ? "bg-white dark:bg-[#103a64] text-[#112a46] dark:text-white shadow-sm" 
                         : "text-slate-500 dark:text-[#8ba4b3] hover:text-[#112a46] dark:hover:text-white"
                     )}
                   >
                     {mode === "all" ? "All Events" : mode === "upcoming" ? "Upcoming" : "Past"}
                   </button>
                 ))}
              </div>
            </div>
